...
def preferred_ml_updates(d):
    ...
    for pkg, reason in blacklists.items():
        if pkg.endswith(("-native", "-crosssdk")) or pkg.endswith("-nativesdk") or 'cross-canadian' in pkg:
            continue
        for p in prefixes:
            newpkg = p + "-" + pkg
    ...
    for v in versions:
        ...
        if pkg.endswith(("-native", "-crosssdk")) or pkg.endswith("-nativesdk"):
            continue
    ...
    for prov in providers:
        ...
        if pkg.startswith("virtual/"):
            ...
            virt = "virtual/"
        for p in prefixes:
            if pkg != "kernel" and pkg != "kernel-image-initramfs":
                val = p + "-" + val
    ...
    extramp = []
    for p in mp:
        if p.endswith(("-native", "-crosssdk")) or p.endswith("-nativesdk") or 'cross-canadian' in p:
            continue
        virt = ""
...
python () {
    ...
    if not source_mirror_fetch:
        ...
        for t in ["-native", "-cross", "-cross-initial", "-cross-intermediate",
              "-crosssdk-intermediate", "-crosssdk", "-crosssdk-initial", "-nativesdk",
